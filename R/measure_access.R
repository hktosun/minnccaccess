...
measure_access <- function(method = "hexagon", geography = "census-block-group", geo_year = 2010, acs_year = 2019, hex_side = 1, radius = 10, decay = 4, as_of = "2021-04-01"){

	if(!method %in% c("hexagon", "census")){
		stop('`method` should be "hexagon" or "census"')
	}
	if(method == "hexagon"){
		access_data <- measure_access_hexagon(geography = geography, geo_year = geo_year, acs_year = acs_year, hex_side = hex_side, radius = radius, decay = decay, as_of = as_of)
	} else if(method == "census"){
		access_data <- measure_access_census(geography = geography, geo_year = geo_year, acs_year = acs_year, radius = radius, decay = decay, as_of = as_of)
	}
