...
measure_access <- function(geography = "census-tract", year = 2010, as_of = "2021-04-01", acs_year = 2019, radius = 10, decay = 4){
	...
	distances <- get_distance_to_providers(geography = geography, year = year, destination = "licensing", as_of = as_of) %>%
		dplyr::select(!!rlang::sym(geographic_var) := .data$id, license_id = .data$id2, .data$distance) %>%
	...
	seats_per_kid <- seats_per_kid %>%
		dplyr::mutate(seat_per_kid = dplyr::case_when(
			seat_per_kid >= quantile(seats_per_kid$seat_per_kid, 0.97) ~ quantile(seats_per_kid$seat_per_kid, 0.97),
			seat_per_kid <= quantile(seats_per_kid$seat_per_kid, 0.03) ~ quantile(seats_per_kid$seat_per_kid, 0.03),
			TRUE ~ seat_per_kid
		))
